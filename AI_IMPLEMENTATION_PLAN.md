...
> **For AI agents:** This is the single source of truth for all V4 implementation work.
> Before starting any pass, read this file. When a pass is complete, update the status.
> Never change the architecture. Never invent new services. Extend what exists.
> If you find a critical problem, document it in the "Issues" section at the bottom — do not fix it silently.

---

## How to use this file

1. Find the first pass with status `[ ] NOT STARTED` or `[~] IN PROGRESS`
2. Read the full pass description — files to change, files to create, components to reuse
3. Implement the entire pass in one session (it is designed to be logically complete)
4. Update the pass status to `[x] DONE` and fill in the completion note
5. Do not start the next pass until the current one compiles and the app runs
