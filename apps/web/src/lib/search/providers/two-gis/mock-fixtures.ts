/**
 * Mock 2GIS API fixtures — realistic JSON that mirrors the real 2GIS
 * Catalog API v3.0 response shape.
 *
 * These fixtures are used by MockTwoGISClient to return believable data
 * while the real API key is not yet configured. The shape is intentionally
 * faithful to the real API so that switching to RealTwoGISClient requires
 * zero changes to TwoGISMapper.
 *
 * Structure mirrors what the real API returns for:
 *   GET /3.0/items?q=<query>&city_id=<id>&type=branch
 *       &fields=items.org,items.rubrics,items.contact_groups,items.links,items.adm_div
 */

import type { TwoGISApiResponse, TwoGISItem } from './types'
...
const CONSTRUCTION_ITEMS: TwoGISItem[] = [
  {
    ...
    address: {
      name: 'Москва, ул. Тверская, 18/1',
      postcode: '125009',
      country_code: 'RU',
      components: [
        { type: 'city', value: 'Москва' },
        { type: 'street', value: 'ул. Тверская' },
        { type: 'house', value: '18/1' },
      ],
    },
  ...
  },
  {
    ...
    address: {
      name: 'Москва, Ленинградский пр-т, 80',
      postcode: '125315',
      country_code: 'RU',
      components: [
        { type: 'city', value: 'Москва' },
        { type: 'street', value: 'Ленинградский пр-т' },
        { type: 'house', value: '80' },
      ],
    },
    adm_div: [{ type: 'city', name: 'Москва', id: '4504222888030252' }],
    org: { id: 'org-002', name: 'АО «КапиталСтрой Групп»', inn: '7703222002', ogrn: '1027700234502', legal_form: 'АО' },
    contact_groups: [{
      contacts: [
  ...
  },
  {
    ...
    rubrics: [
      { id: '164', name: 'Строительные компании', short_name: 'Строительство', is_main_in_system: true },
      { id: '166', name: 'Промышленное строительство' },
    ],
    address: {
      name: 'Москва, ул. Садовая-Черногрязская, 3',
      postcode: '107078',
      country_code: 'RU',
      components: [
        { type: 'city', value: 'Москва' },
        { type: 'street', value: 'ул. Садовая-Черногрязская' },
        { type: 'house', value: '3' },
      ],
    },
    adm_div: [{ type: 'city', name: 'Москва', id: '4504222888030252' }],
...
const LOGISTICS_ITEMS: TwoGISItem[] = [
  {
    ...
    address: {
      name: 'Екатеринбург, ул. Малышева, 51',
      postcode: '620000',
      country_code: 'RU',
      components: [
        { type: 'city', value: 'Екатеринбург' },
        { type: 'street', value: 'ул. Малышева' },
        { type: 'house', value: '51' },
      ],
    },
  ...
  },
  {
    ...
    address: {
      name: 'Екатеринбург, пр-т Ленина, 25',
      postcode: '620014',
      country_code: 'RU',
      components: [
        { type: 'city', value: 'Екатеринбург' },
        { type: 'street', value: 'пр-т Ленина' },
        { type: 'house', value: '25' },
      ],
    },
    adm_div: [{ type: 'city', name: 'Екатеринбург', id: '4504222888030765' }],
  ...
  },
  {
    ...
    rubrics: [
      { id: '203', name: 'Логистические компании', is_main_in_system: true },
      { id: '204', name: 'Складские услуги' },
    ],
    address: {
      name: 'Екатеринбург, ул. Цвиллинга, 10',
      postcode: '620137',
      country_code: 'RU',
      components: [
        { type: 'city', value: 'Екатеринбург' },
        { type: 'street', value: 'ул. Цвиллинга' },
        { type: 'house', value: '10' },
      ],
    },
    adm_div: [{ type: 'city', name: 'Екатеринбург', id: '4504222888030765' }],
...
const LEGAL_ITEMS: TwoGISItem[] = [
  {
    ...
    rubrics: [
      { id: '301', name: 'Юридические компании', short_name: 'Юридические услуги', is_main_in_system: true },
    ],
    address: {
      name: 'Казань, ул. Баумана, 44',
      postcode: '420000',
      country_code: 'RU',
      components: [
        { type: 'city', value: 'Казань' },
        { type: 'street', value: 'ул. Баумана' },
        { type: 'house', value: '44' },
      ],
    },
...
/**
 * Returns a mock 2GIS API response based on the query string and city_id.
 * The selection logic mirrors what the real API would return for each query.
 */
export function getMockResponse(
  ...
  // Determine which fixture set to use based on query keywords
  let items: TwoGISItem[]

  if (q.includes('строи') || q.includes('подряд') || q.includes('ремонт')) {
    items = cityId === '4504222888030765'  // Екатеринбург
      ? CONSTRUCTION_ITEMS.slice(2)       // ПромСтрой-like
      : CONSTRUCTION_ITEMS
  } else if (
    q.includes('логист') ||
    q.includes('транспорт') ||
    q.includes('перевоз') ||
    q.includes('склад') ||
    q.includes('груз')
  ) {
