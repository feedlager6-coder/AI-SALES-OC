 ...
 * betterAuth() calls getDb() which reads DATABASE_URL. During `next build`,
 * Next.js executes route-handler modules to collect their exports; if auth
 * were initialized at module-load time, the build would fail whenever
 * DATABASE_URL is absent from the build environment. getAuth() defers
 * initialization to the first incoming request, so the build always succeeds.
 */
...
// Use `any` storage to avoid TypeScript's contravariant generic mismatch between
// Auth<ConcreteOptions> (what betterAuth() returns) and Auth<BetterAuthOptions>
// (what ReturnType<typeof betterAuth> resolves to at the call site).
// The exported getAuth() return type is still structurally correct for callers.
...
/**
 * Returns the singleton Better Auth instance.
 * Initializes on first call (request time), never at module load (build time).
 */
export function getAuth(): ReturnType<typeof betterAuth> {
  ...
  _auth = betterAuth({
    ...
    // (e.g. https://ai-sales-osweb-production.up.railway.app)
