/**
 * RuleBasedIntentParser — no LLM, no network.
 * Parses a Russian natural-language query into structured intent fields
 * using keyword dictionaries and regular expressions.
 *
 * REPLACING THIS IN THE FUTURE:
 *   Create an AIIntentParser that implements IntentParser and calls your
 *   LLM endpoint. Pass it to IntentService — no other changes needed.
 */

import type { IntentParser, ParsedIntentResult, ClarifyingQuestion } from './intent-parser.interface.js'
...
export class RuleBasedIntentParser implements IntentParser {
  parse(query: string): ParsedIntentResult {
    ...
    // Company size — range pattern first, then from/to
    let companySize: string | null = null
    const rangeMatch = query.match(
      /(\d[\d\s]*)\s*[–\-—]\s*(\d[\d\s]*)\s*(сотрудник|человек|чел\.?|раб\.?)/i,
    )
    if (rangeMatch) {
      companySize = `${rangeMatch[1]!.trim()}–${rangeMatch[2]!.trim()} сотрудников`
    } else {
    ...
    // Clarifying question — skip when query already mentions newness
